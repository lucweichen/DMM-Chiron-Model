    ...
    public void workflowUpdate() throws SQLException {
        ...
        if (workflowCheck() == true) {
            ...
            if (wish.equalsIgnoreCase("y")) {
                q = "UPDATE cworkflow SET description=? WHERE tag=?";
        ...
        if (wish2.equalsIgnoreCase("y")) {
            for (Activity a : newActivities) {
                q = "INSERT INTO cactivity(actid, wkfid, tag, atype, description, templatedir, activation, extractor, constrained)VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                query = db.prepQuery(q);
                ...
                for (Relation r : a.getRelations()) {
                    if (r.getDependencyUp() == null) {
                        q = "INSERT INTO crelation(relid, actid, rtype, rname) VALUES (?, ?, ?, ?)";
                        query = db.prepQuery(q);
                        ...
                        q = "INSERT INTO crelation(relid, actid, rtype, rname, dependency) VALUES (?, ?, ?, ?, ?)";
                        query = db.prepQuery(q);
                        query.setParInt(1, r.getRelid());
                        query.setParInt(2, r.getActid());
                        query.setParString(3, r.getRtype().toUpperCase());
                ...
                for (Relation r : a.getRelations()) {
                    for (Field f : r.getFields()) {
                        q = "INSERT INTO cfield(fname, relid, ftype, decimalplaces, fileoperation, instrumented) VALUES (?, ?, ?, ?, ?, ?)";
                        query = db.prepQuery(q);
            ...
            this.setDependencyRelations();
            for (Activity a : updateActivities) {
                q = "UPDATE cactivity SET atype=?, description=?, templatedir=?, activation=?, extractor=?, constrained=? WHERE tag=?";
                query = db.prepQuery(q);
        ...
        if (wish3.equalsIgnoreCase("y")) {
            ...
            for (Relation r : updateRelations) {
                q = "UPDATE crelation SET relid=?, actid=?, rtype=?, rname=?, dependency=? WHERE relid=?";
                query = db.prepQuery(q);
                query.setParInt(1, r.getRelid());
                query.setParInt(2, r.getActid());
                query.setParString(3, r.getRtype().toUpperCase());
            ...
            for (Activity a : this.newActivities) {
                for (Relation r : a.getRelations()) {
                    if ((r.getRtype().equalsIgnoreCase("input")) && (r.getDependencyUp() == null)) {
                        q = "CREATE SEQUENCE " + r.getRname().toLowerCase() + "_seq";
                        query = db.prepQuery(q);
                        query.executeUpdate();
                    }
        ...
        if (wish2.equalsIgnoreCase("y")) {
            for (Activity a : this.newActivities) {
                for (Relation r : a.getRelations()) {
                    if (r.getRtype().equalsIgnoreCase("input")) {
                        if (r.getDependencyUp() == null) {
                            q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT " + r.getRname() + "_pkey PRIMARY KEY (ewkfid, ik)";
                            query = db.prepQuery(q);
                            query.executeUpdate();
                        } else {
                            q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT " + r.getRname() + "_pkey PRIMARY KEY (ewkfid, ik)";
                                ...
                                if (r.getDependencyUp().equalsIgnoreCase(act.getTag())) {
                                    for (Relation rel : act.getRelations()) {
                                        if (rel.getRtype().equalsIgnoreCase("output")) {
                                            q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT \"" + r.getRname() + "_fk\" FOREIGN KEY (ewkfid, ik) REFERENCES \"" + rel.getRname() + "\" (ewkfid, ok) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE";
                                            query = db.prepQuery(q);
                                            query.executeUpdate();
                                        }
                                    }
                    ...
                    if (r.getRtype().equalsIgnoreCase("output")) {
                        q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT " + r.getRname() + "_pkey PRIMARY KEY (ewkfid, ok)";
                        query = db.prepQuery(q);
                        query.executeUpdate();
                        for (Relation rel : a.getRelations()) {
                            if (rel.getRtype().equalsIgnoreCase("input")) {
                                q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT \"" + r.getRname() + "_fk\" FOREIGN KEY (ewkfid, ik) REFERENCES \"" + rel.getRname() + "\" (ewkfid, ik) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE";
                                query = db.prepQuery(q);
                                query.executeUpdate();
                            }
                        }
                    ...
                    if (r.getDependencyUp() == null) {
                        ...
                        for (Activity act : xml.getActivities()) {
                            if (r.getDependencyUp().equalsIgnoreCase(act.getTag())) {
                                for (Relation rel : act.getRelations()) {
                                    if (rel.getRtype().equalsIgnoreCase("output")) {
                                        q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT \"" + r.getRname() + "_fk\" FOREIGN KEY (ewkfid, ik) REFERENCES \"" + rel.getRname() + "\" (ewkfid, ok) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE NOT VALID";
                                        query = db.prepQuery(q);
                                        query.executeUpdate();
                                    }
                                }
            ...
            for (Relation r : this.updateRelations) {
                for (Activity act : xml.getActivities()) {
                    if (r.getDependencyUp().equalsIgnoreCase(act.getTag())) {
                        for (Relation rel : act.getRelations()) {
                            if (rel.getRtype().equalsIgnoreCase("output")) {
                                q = "ALTER TABLE \"" + r.getRname() + "\" DROP CONSTRAINT \"" + r.getRname() + "_fk\"";
                                query = db.prepQuery(q);
                                query.executeUpdate();
                                q = "ALTER TABLE \"" + r.getRname() + "\" ADD CONSTRAINT \"" + r.getRname() + "_fk\" FOREIGN KEY (ewkfid, ik) REFERENCES \"" + rel.getRname() + "\" (ewkfid, ik) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE NOT VALID";
                                query = db.prepQuery(q);
                                query.executeUpdate();
                            }
                        }
                                ...
                                for (Relation rel : act.getRelations()) {
                                    if ((rel.getRtype().equalsIgnoreCase("input")) && (rel.getDependencyUp() != null)) {
                                        if (rel.getDependencyUp().equalsIgnoreCase(a.getTag())) {
                                            q = "ALTER TABLE \"" + rel.getRname() + "\" DROP CONSTRAINT \"" + rel.getRname() + "_fk\"";
                                            query = db.prepQuery(q);
                                            query.executeUpdate();
                                            q = "ALTER TABLE \"" + rel.getRname() + "\" ADD CONSTRAINT \"" + rel.getRname() + "_fk\" FOREIGN KEY (ewkfid, ik) REFERENCES \"" + r.getRname() + "\" (ewkfid, ok) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE NOT VALID";
                                            query = db.prepQuery(q);
                                            query.executeUpdate();
                                        }
                                    }
                            ...
                            for (Relation rel : a.getInput()) {
                                if (rel.getRname().equalsIgnoreCase(r.getRname())) {
                                    for (Relation rela : a.getOutput()) {
                                        q = "ALTER TABLE \"" + rela.getRname() + "\" DROP CONSTRAINT \"" + rela.getRname() + "_fk\"";
                                        query = db.prepQuery(q);
                                        query.executeUpdate();
                                        q = "ALTER TABLE \"" + rela.getRname() + "\" ADD CONSTRAINT \"" + rela.getRname() + "_fk\" FOREIGN KEY (ewkfid, ik) REFERENCES \"" + r.getRname() + "\" (ewkfid, ik) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE NOT VALID";
                                        query = db.prepQuery(q);
                                        query.executeUpdate();
                                    }
                                }
        ...
        if (wish4.equalsIgnoreCase("y")) {
            for (Field f : newFields) {
                ...
                q = "ALTER TABLE \"" + rname + "\" ADD COLUMN " + f.getFname() + " " + f.getFtypetoSQL();
                query = db.prepQuery(q);
                query.executeUpdate();
            }
            for (Field f : updateFields) {
                q = "UPDATE cfield SET fname=?, relid=?, ftype=?, decimalplaces=?, fileoperation=?, instrumented=? WHERE fname=? AND relid=?";
                query = db.prepQuery(q);
                query.setParString(1, f.getFname());
                query.setParInt(2, f.getRelid());
                query.setParString(3, f.getFtype());
            ...
            for (Field f : deleteFields) {
                q = "DELETE FROM cfield WHERE fname=? AND relid=?";
                query = db.prepQuery(q);
                query.setParString(1, f.getFname());
                query.setParInt(2, f.getRelid());
                query.executeUpdate();
                for (Relation r : this.xmlRelations) {
                    if (r.getRelid() == f.getRelid()) {
                        q = "ALTER TABLE " + r.getRname() + " DROP COLUMN " + f.getFname();
                        query = db.prepQuery(q);
                        query.executeUpdate();
                    }
                }
