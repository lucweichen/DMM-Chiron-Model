...
public class Relation {
    ...
    public Relation(String name, Type type) {
        this.rname = name;
        this.type = type;
        this.fields = new ArrayList<Field>();
    }

    public Relation(String rtype, String rname, String dependencyUp) {
        this.rtype = rtype;
        this.rname = rname;
    ...
    public Relation(int relid, int actid, String rtype, String rname, int dependencyDb) {
        this.relid = relid;
        this.actid = actid;
        this.rtype = rtype;
        this.rname = rname;
    ...
    public String getSQLCreateStatement() {
        ...
        String statement = "CREATE TABLE \"" + this.getRname() + "\" ( " + separator;
    ...
    private String workflowInputCreateStatement() {
        ...
        String statement = "CREATE TABLE " + this.getRname() + " ( " + separator;
        statement += "ewkfid integer NOT NULL," + separator;
        statement += "ik integer DEFAULT nextval(('" + this.getRname().toLowerCase() + "_seq'::text)::regclass) NOT NULL," + separator;
    ...
    private String outputCreateStatement() {
        ...
        statement += "ewkfid integer NOT NULL," + separator;
        statement += "ik integer NOT NULL," + separator
                + "ok integer DEFAULT nextval(('" + this.getRname().toLowerCase() + "_seq'::text)::regclass) NOT NULL," + separator;
        for (int i = 0; i < fields.size(); i++) {
            if (i != 0) {
                statement += "," + separator;
            }
    ...
    public String getInputFKSQLStatement(Relation r) {
        ...
        statement += "," + separator + "CONSTRAINT " + this.getRname() + "_pkey PRIMARY KEY (ewkfid, ik)";
//        statement += "," + separator + "CONSTRAINT " + this.getRname() + "_fk FOREIGN KEY (ewkfid, ik) REFERENCES " + r.getRname() + " (ewkfid, ok) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE";
        statement += separator + ")";
        return statement;
    }
...
//        statement += "," + separator + "CONSTRAINT " + this.getRname() + "_fk FOREIGN KEY (ewkfid, ik) REFERENCES " + r.getRname() + " (ewkfid, ik) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE";
        statement += separator + ")";
        return statement;
    }
    ...
    public String getSQLSequence() {
        String separator = System.getProperty("line.separator");
        String statement = separator + "CREATE SEQUENCE " + this.getRname().toLowerCase() + "_seq" + separator + "INCREMENT BY 1" + separator + "NO MAXVALUE" + separator + "MINVALUE 0" + separator + "START 0" + separator + "CACHE 1;" + separator;;
        return statement;
    }

    public String startSequence() {
        String statement = "SELECT pg_catalog.nextval('" + this.getRname().toLowerCase() + "_seq')";
        return statement;
    }
    ...
    public String getSQLUpdate() {
        ...
        if (this.type.equals(Type.OUTPUT)) {
            statement = "ALTER TABLE " + this.getRname()
                    + " CLUSTER ON " + this.getRname() + "_pkey;" + separator
                    + "CREATE UNIQUE INDEX " + this.getRname() + "_index ON "
                    + this.getRname() + " USING btree (ik, ok);" + separator
                    + "CREATE UNIQUE INDEX " + this.getRname() + "_key_index ON "
                    + this.getRname() + " USING btree (ok);";
        } else {
            statement = "ALTER TABLE " + this.getRname()
                    + " CLUSTER ON " + this.getRname() + "_pkey;" + separator
                    + "CREATE UNIQUE INDEX " + this.getRname() + "_key_index ON "
                    + this.getRname() + " USING btree (ik);";
        }
        return statement;
    }
