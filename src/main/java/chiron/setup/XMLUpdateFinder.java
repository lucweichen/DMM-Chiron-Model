...
public class XMLUpdateFinder {
    ...
    public void workflowUpdate(String xmlfile) {
        ...
        try {
            ...
            Element Workflow = doc.getRootElement().getFirstChildElement("Workflow");
            ...
            for (int i = 0; i < activities.size(); i++) {
                ...
                Activity act = new Activity(activity.getAttributeValue("tag"), activity.getAttributeValue("type"), activity.getAttributeValue("description"), activity.getAttributeValue("templatedir"), activity.getAttributeValue("activation"), activity.getAttributeValue("extractor"), activity.getAttributeValue("constrained"), activity.getAttributeValue("workload"));
                ...
                for (int j = 0; j < activityElements.size(); j++) {
                    ...
                    if (elementName.equalsIgnoreCase("relation")) {
                        if (element.getAttributeValue("reltype").equalsIgnoreCase("input")) {
                            ...
                            Relation relation = new Relation(type, name, dependency);
