...
public class DBReader {
    ...
    public void retrieveWorkflow(String tag) throws SQLException {
        String q = "SELECT * FROM cworkflow WHERE tag = ?";
    ...
    public void retrieveActivities(int wkfid) throws SQLException {
        String q = "SELECT * FROM cactivity WHERE wkfid = ?";
        M_Query query = db.prepQuery(q);
        query.setParInt(1, wkfid);
        ResultSet r = query.openQuery();
        while (r.next()) {
            Activity a = new Activity(r.getInt("actid"), r.getInt("wkfid"), r.getString("tag"), r.getString("atype"), r.getString("description"), r.getString("templatedir"), r.getString("activation"), r.getString("extractor"), r.getString("constrained"));
            this.activities.add(a);
        }
    }

    public void retrieveRelations(int actid) throws SQLException {
        String q = "SELECT * FROM crelation WHERE actid = ?";
        M_Query query = db.prepQuery(q);
        query.setParInt(1, actid);
        ResultSet r = query.openQuery();
        while (r.next()) {
            Relation rel = new Relation(r.getInt("relid"), r.getInt("actid"), r.getString("rtype"), r.getString("rname"), r.getInt("dependency"));
    ...
    public void retrieveFields(int relid) throws SQLException {
        String q = "SELECT * FROM cfield WHERE relid = ?";
        M_Query query = db.prepQuery(q);
    ...
    public int setIdActivities() throws SQLException {
        int index = 0;
        String q = "SELECT * FROM cactivity";
        M_Query query = db.prepQuery(q);
        ResultSet r = query.openQuery();
        while (r.next()) {
    ...
    public int setIdRelations() throws SQLException {
        int index = 0;
        String q = "SELECT * FROM crelation";
        M_Query query = db.prepQuery(q);
        ResultSet r = query.openQuery();
        while (r.next()) {
