...
public class DBHandler {
    ...
    private boolean workflowExists(Workflow w) throws SQLException {
        String sql = "SELECT * FROM cworkflow WHERE tag=?";
    ...
    private List<Relation> existingRelations(Map<String, Activity> activities) throws SQLException {
        ...
        List<String> tables = this.retriveTables();
        ...
        if (this.workflowExists(w)) {
            ...
            if (existingRelations.size() > 0) {
                ...
                for (Activity a : w.activities.values()) {
                    ...
                    q.setParDouble(9, a.getWorkload());
                    rs = this.openQuery(q);
                    if (rs.next()) {
                        a.setId(rs.getInt(1));
                    } else {
    ...
    private void insertFields(Relation relation) throws SQLException {
        for (Field f : relation.getFields()) {
            String sql = "INSERT INTO cfield(fname, relid, ftype, decimalplaces, fileoperation, instrumented) VALUES(?,?,?,?,?,?)";
            M_Query q = db.prepQuery(sql);
    ...
    private void insertOperands(Activity a) throws SQLException {
        for(Operand op : a.getOperands()){
            String sql = "INSERT INTO coperand(actid, oname, numericvalue, textvalue) VALUES(?,?,?,?)";
            M_Query q = db.prepQuery(sql);
    ...
    void deleteWorkflow(Workflow w, Reader reader) throws SQLException {
        String wkftag = w.getTag();
        DBReader r = new DBReader(reader.getDbAttribute("server"),
        ...
        String dropWorkflow = "DELETE FROM cworkflow WHERE tag=?";
        M_Query q = db.prepQuery(dropWorkflow);
        q.setParString(1, w.getTag());
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM ifsite";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM site";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM vm";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM connection";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM eactivation";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM eactivity";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM efile";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM ekeyspace";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM emachine";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
        dropWorkflow = "DELETE FROM eworkflow";
        q = db.prepQuery(dropWorkflow);
        this.executeUpdate(q);
//        for (Activity a : w.activities.values()) {
//            String sql = a.getDropStatement();
...
//        this.executeUpdate(q);
     ...
     * @throws SQLException
     */
    public List<String> retriveTables() throws SQLException {
